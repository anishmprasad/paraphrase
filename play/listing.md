...
## Notes for the reviewer
    ...
    2. Type a sentence into the "Try it" field at the bottom.
    3. Select the text, tap the three-dot overflow in the selection toolbar,
       and tap Paraphrase.

    No account or API key is needed to review the app: with no key configured
    it falls back to an on-device rewriter, so every screen is reachable
    without credentials. Adding a free Gemini or Groq key switches it to a
    real model.
